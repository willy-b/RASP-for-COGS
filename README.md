...
Code for RASP-for-COGS described in the paper "Exploring Compositional Generalization (in COGS/ReCOGS_pos)
by Transformers using Restricted Access Sequence Processing (RASP)" ( https://arxiv.org/abs/2504.15349 ).

A demonstration using Restricted Access Sequence Processing (RASP) [(Weiss et al., 2021)](https://proceedings.mlr.press/v139/weiss21a.html), a Transformer-equivalent programming language, of how a Transformer Encoder-Decoder with a fixed depth representation, i.e. without tree-structured/hierarchical representations (and without tree-structuring the model), can perform [Kim and Linzen 2020's COGS task](https://aclanthology.org/2020.emnlp-main.731/) including recursive aspects of the grammar by unrolling the input depth in the decoder loop ([see figures at the bottom of this README](https://github.com/willy-b/RASP-for-COGS#rasp-for-cogs-encoder-decoder-schematic-simplified) or the [paper](https://arxiv.org/abs/2504.15349) for details).

We argue in the [paper](https://arxiv.org/abs/2504.15349) that avoiding tree-structured representations which, in a vanilla Transformer, to be compositional may require activating a number of Transformer layers proportional to the tree depth [(Csordás et al., 2022)](https://openreview.net/forum?id=KBQP4A_J1K) will make the model more generalizable. This is important because structural generalization errors made by Transformers are well-known [(Yao and Koller, 2022)](https://doi.org/10.18653/v1/2022.emnlp-main.337). A tree-structured representation in a vanilla Transformer is not expected to generalize to tree depths unseen in training as that would require activating deeper layers than were activated in training, whereas an unrolling approach can by reapplying a fixed depth representation repeatedly. Others propose to solve this by tree-structuring the model [(Kim and Linzen, 2020)](https://aclanthology.org/2020.emnlp-main.731/), [(Yao and Koller, 2022)](https://doi.org/10.18653/v1/2022.emnlp-main.337), [(Liu et al., 2021)](https://doi.org/10.18653/v1/2021.findings-acl.97) or by weight sharing across layers [(Csordás et al., 2022)](https://openreview.net/forum?id=KBQP4A_J1K) [(Dehghani et al., 2019)](https://openreview.net/forum?id=HyzdRiR9Y7), but we try to show that such architecture changes are unnecessary. See [the literature review here](https://github.com/willy-b/RASP-for-COGS#more-background--literature-review) or [in our paper](https://arxiv.org/abs/2504.15349) for a list of papers supporting this argument.
...
See also our finished RASP solution to the semantically equivalent ReCOGS task (Wu et al 2023, "ReCOGS: How Incidental Details of a Logical Form Overshadow an Evaluation of Semantic Interpretation", https://doi.org/10.1162/tacl_a_00623 ) in the RASP-for-ReCOGS code linked from the combined RASP-for-COGS/ReCOGS paper at https://arxiv.org/abs/2504.15349 (RASP-for-COGS/ReCOGS results are reported together as of the version uploaded in 2025-10).
...
These are intended be obvious or self-evident conditions, but especially for those who do not read the associated RASP-for-COGS/ReCOGS paper ( https://arxiv.org/abs/2504.15349 ):
...
SOFTWARE. See the [full license](LICENSE) .
